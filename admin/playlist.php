...
function onair_PlaylistSave()
{
    ...
    $xoopsDB->query('UPDATE '
                    . $xoopsDB->prefix('oa_playlist')
                    . ' SET pl_day = '
                    . $xoopsDB->quoteString($_POST['pl_day'])
                    . ', pl_station = '
                    . $xoopsDB->quoteString($_POST['pl_station'])
                    . ', pl_title = '
                    . $xoopsDB->quoteString($_POST['pl_title'])
                    . ', pl_name = '
                    . $xoopsDB->quoteString($_POST['pl_name'])
                    . ', pl_start = '
                    . $xoopsDB->quoteString($_POST['pl_start'])
                    . ', pl_stop = '
                    . $xoopsDB->quoteString($_POST['pl_stop'])
                    . ', pl_image = '
                    . $xoopsDB->quoteString($_POST['pl_image'])
                    . ', pl_description = '
                    . $xoopsDB->quoteString($_POST['pl_description'])
                    . ', pl_date = '
                    . $xoopsDB->quoteString($_POST['pl_date'])
                    . ' WHERE pl_id = '
                    . (int)$_POST['pl_id']
                    . '');
 ...
 *
 * @param $pl_id
 * @internal param \Place $In admin Load data to edit form and send to onair_PlaylistSave
 * @internal param int $repeat 1
 *
 * @return Status
 */
function onair_PlaylistEdit($pl_id)
{
    ...
    $result       = $xoopsDB->query('SELECT pl_day, pl_title, pl_name, pl_start, pl_stop, pl_image, pl_date, pl_description,pl_station FROM ' . $xoopsDB->prefix('oa_playlist') . ' WHERE pl_id=' . (int)$pl_id . '');
    ...
    $playlistformdescription = new XoopsFormDhtmlTextArea(_AM_ONAIR_DESCRIPTION, 'pl_description', $myts->htmlSpecialChars($myts->stripSlashesGPC($pl_description)), 10, 50);
...
function onair_PlaylistShow()
{
    ...
    $result = $xoopsDB->query('SELECT pl_id, pl_day, pl_title, pl_start, pl_stop ,' . ' pl_image, pl_description, pl_date FROM ' . $xoopsDB->prefix('oa_playlist') . ' ORDER BY pl_date,pl_start');
 ...
 *
 * @param $oa_id
 * @internal param \Place $In admin choose event to make playlist to
 * @internal param int $repeat 1
 *
 * @return Status
 */
function onair_Playlist($oa_id)
{
    ...
    $result = $xoopsDB->query('SELECT oa_id, oa_day, oa_title, oa_name, oa_start, oa_stop, oa_image,oa_station FROM ' . $xoopsDB->prefix('oa_program') . ' WHERE oa_id=' . (int)$oa_id . '');
    $myts   = MyTextSanitizer::getInstance();
...
switch ($op) {
    ...
    case 'Playlistpost':
        ...
        $sqlinsert = 'INSERT INTO '
                     . $xoopsDB->prefix('oa_playlist')
                     . " (pl_id, pl_day, pl_title, pl_date, pl_start, pl_stop,pl_description, pl_image, pl_name, pl_station) VALUES ('', "
                     . $xoopsDB->quoteString($pl_day)
                     . ', '
                     . $xoopsDB->quoteString($pl_title)
                     . ', '
                     . $xoopsDB->quoteString($pl_date)
                     . ', '
                     . $xoopsDB->quoteString($pl_start)
                     . ', '
                     . $xoopsDB->quoteString($pl_stop)
                     . ', '
                     . $xoopsDB->quoteString($pl_description)
                     . ', '
                     . $xoopsDB->quoteString($pl_image)
                     . ', '
                     . $xoopsDB->quoteString($pl_name)
                     . ', '
                     . $xoopsDB->quoteString($pl_station)
                     . ')';
