 ...
 * @param int $del
 *
 * @internal param \Place $In admin delete chart from database
 * @internal param int $repeat 1
 *
 * @return Status
 */
 ...
 *
 * @internal param \Place $In admin saves your edited chart
 * @internal param int $repeat 1
 *
 * @return Status
 */
function onair_ChartSave()
{
    ...
    $xoopsDB->query('UPDATE '
                    . $xoopsDB->prefix('oa_charts')
                    . ' SET ch_charttitle = '
                    . $xoopsDB->quoteString($_POST['ch_charttitle'])
                    . ', ch_chartsong = '
                    . $xoopsDB->quoteString($_POST['ch_chartsong'])
                    . ', ch_songweekstotal = '
                    . $xoopsDB->quoteString($_POST['ch_songweekstotal'])
                    . ', ch_songlastweek = '
                    . $xoopsDB->quoteString($_POST['ch_songlastweek'])
                    . ', ch_songtopplace = '
                    . $xoopsDB->quoteString($_POST['ch_songtopplace'])
                    . ', ch_songthisweek = '
                    . $xoopsDB->quoteString($_POST['ch_songthisweek'])
                    . ', ch_songweek = '
                    . $xoopsDB->quoteString($_POST['ch_songweek'])
                    . ', ch_songplaytime = '
                    . $xoopsDB->quoteString($_POST['ch_songplaytime'])
                    . ' WHERE ch_chartid = '
                    . (int)$_POST['ch_chartid']
                    . '');
 ...
 *
 * @param $ch_chartid
 * @internal param \Place $In admin Load data to edit form and send to onair_ChartSave
 * @internal param int $repeat 1
 *
 * @return Status
 */
function onair_ChartEdit($ch_chartid)
{
    ...
    $result    = $xoopsDB->query('SELECT ch_chartid, ch_charttitle, ch_chartsong, ch_chartweekstotal, ch_chartlastweek, ch_charttopplace, ch_chartthisweek, ch_chartweek ,ch_chartplaytime FROM '
                                 . $xoopsDB->prefix('oa_chart')
                                 . ' WHERE ch_chartid='
                                 . (int)$ch_chartid
                                 . ' ORDER BY ch_chartthisweek LIMIT 0,20');
...
function onair_ChartShow()
{
    ...
    echo "</b></td></div><table border='0' width='100%' class='outer' align='center'>
        <tr><td class='even'><b>"
         ...
         . _AM_ONAIR_CHARTLASTWEEK
         . "</b></td><td class='even'><b>"
         . _AM_ONAIR_CHARTTOPPLACE
         . "</b></td><td class='even'><b>"
...
function onair_Chart($ch_week)
{
    ...
    $result = $xoopsDB->query('SELECT SELECT oa_songsong, COUNT( * ) FROM ' . $xoopsDB->prefix('oa_hitlist') . ' WHERE oa_songweek=' . (int)$ch_week . ' GROUP BY oa_songsong ORDER BY COUNT( * ) DESC LIMIT 0 , 20;');
    $myts   = MyTextSanitizer::getInstance();
...
switch ($op) {
    ...
    case 'chartpost':
        ...
        $sqlinsert = 'INSERT INTO '
                     . $xoopsDB->prefix('oa_chart')
                     . " (ch_chartid, ch_charttitle, ch_chartsong, ch_chartweekstotal, ch_chartlastweek, ch_charttopplace,ch_chartthisweek, ch_chartweek, ch_chartplaytime) VALUES ('', "
                     . $xoopsDB->quoteString($charttitle)
                     . ', '
                     . $xoopsDB->quoteString($chartsong)
                     . ', '
                     . $xoopsDB->quoteString($chartweekstotal)
                     . ', '
                     . $xoopsDB->quoteString($chartlastweek)
                     . ', '
                     . $xoopsDB->quoteString($charttopplace)
                     . ', '
                     . $xoopsDB->quoteString($chartthisweek)
                     . ', '
                     . $xoopsDB->quoteString($chartweek)
                     . ', '
                     . $xoopsDB->quoteString($chartplaytime)
                     . ')';
