 ...
 * @param $oa_day
 *
 * @internal param \Place $User side
 ...
 *
 * @param $oa_day
 * @internal param \Place $Admin side
 * @internal param int $repeat 1
 *
 * @return Status
 */
...
function onair_ChartLastWeek($oa_songsong, $oa_songweek, $year)
{
    ...
    $a      = 1;
    $query  = 'SELECT oa_songsong, count( * ) FROM ' . $xoopsDB->prefix('oa_hitlist') . ' WHERE oa_songweek = ' . (int)$lastweek . ' AND oa_songyear = ' . (int)$year . ' GROUP BY oa_songsong ORDER BY COUNT( * ) DESC';
    $result = $xoopsDB->query($query);
    $i      = $xoopsDB->getRowsNum($result);
    while ($sqlfetch = $xoopsDB->fetchArray($result)) {
...
function onair_GetThisWeekPos($weeknr, $songdata, $year)
{
    ...
    $myts   = MyTextSanitizer::getInstance();
    $sql    = 'SELECT oa_songsong, count(*) FROM ' . $xoopsDB->prefix('oa_hitlist') . ' WHERE oa_songweek = ' . (int)$weeknr . ' AND oa_songyear = ' . (int)$year . ' GROUP BY oa_songsong ORDER BY count(*) DESC LIMIT 20';
...
function onair_WeeksTotal($song, $week, $year)
{
    global $xoopsDB, $myts;
    $myts   = MyTextSanitizer::getInstance();
    $sql    = 'SELECT COUNT(DISTINCT oa_songweek ) AS sum FROM ' . $xoopsDB->prefix('oa_hitlist') . " WHERE oa_songsong = '" . $myts->addSlashes($song) . "' AND oa_songweek <= " . (int)$week . ' AND oa_songyear = ' . $myts->addSlashes($year) . '';
    $result = $xoopsDB->queryF($sql);
    while ($row = $xoopsDB->fetchArray($result)) {
        $sum = $row['sum'];
    }
    ...
    for ($loop = 1; $loop <= $i; ++$loop) {
        ...
        while ($sqlfetch = $xoopsDB->fetchArray($result)) {
            ...
            if ($msg['ch_thisweek'] < $msg['ch_lastweek']) {
                $msg['ch_arrow'] = "<img src = 'assets/images/icons/arrow_up.png'></img>";
            }
            if ($msg['ch_thisweek'] > $msg['ch_lastweek']) {
                $msg['ch_arrow'] = "<img src = 'assets/images/icons/arrow_down.png'></img>";
            }
            if ($msg['ch_thisweek'] == $msg['ch_lastweek']) {
                $msg['ch_arrow'] = "<img src = 'assets/images/icons/nochange.png'></img>";
            }
